...
class BackendSpec:
    ...
    zaya_exe: Optional[Path] = None
    zaya_extra_args: tuple = ()
    zaya_env: dict = field(default_factory=dict)
...
def _build_backend(bid: str, b: dict) -> BackendSpec:
    ...
    if sdcpp is True:                        # shorthand: "sdcpp": true
        sdcpp = {}
    zaya = b.get("zaya")
    if zaya is True:                         # shorthand: "zaya": true
        zaya = {}
    return BackendSpec(
        backend_id=bid,
        ...
        zaya_enabled=zaya is not None,
        zaya_exe=_path((zaya or {}).get("exe")),
        zaya_extra_args=tuple(str(a) for a in (zaya or {}).get("extra_args", [])),
        zaya_env={str(k): str(v) for k, v in ((zaya or {}).get("env") or {}).items()},
    )
...
def zaya_server_exe_for(backend: str) -> Path:
    """The zaya_server binary for a backend: its per-backend build when
    configured, else the global paths.zaya_server_exe."""
    spec = BACKENDS.get(backend)
...
def applies(engine: str, backend: str, model: ModelSpec,
    ...
    if engine == "zaya" and not b.zaya_enabled:
        return False, f"{b.backend_id} has no zaya_server launch mapping"
    ...
    if engine == "zaya":
        if scenario.modality is not None:
            return False, f"zaya has no {scenario.modality} input path"
        if scenario.kind not in ("text", "multi_turn"):
            return False, "zaya runs text / multi_turn scenarios only"
        if mtp:
            return False, "zaya has no MTP speculative decode path"
