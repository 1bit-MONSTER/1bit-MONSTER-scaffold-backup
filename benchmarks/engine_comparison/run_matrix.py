...
def main():
    ...
    for (engine_id, backend, model_id, mtp), cells in groups:
        ...
        elif engine_id == "zaya" and not config.zaya_server_exe_for(backend).exists():
            missing = (f"zaya_server for backend '{backend}' not found: "
                       f"{config.zaya_server_exe_for(backend)}")
