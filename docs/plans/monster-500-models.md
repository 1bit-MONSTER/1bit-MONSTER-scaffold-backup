...
- **MONSTER as product name? — RESOLVED 2026-08-14:** yes. The product is now **1bit MONSTER**, repo `1bit-MONSTER/1bit-MONSTER`, domain `1bit.monster`. "One Bit Systems" / `1bit-systems` is the predecessor identity; the engine's full git history was carried over intact (see repo root README). Gaia (third_party/gaia) and the old hand-rolled `onebit` agent CLI were both removed in the same pass — JARVIS (`tools/jarvis/`) is the one agent/voice pipeline going forward.
