...
> Note added during docs audit: sections 3d and 3e below were physically out of
> order in the original document (3e appeared before 3d, even though 3e's text
> presupposes 3d's root-cause finding, and `tasks.md` cites "findings §3d" for
> the root cause and "findings §3e" for the proven fix, in that order). Reordered
> here to match; no wording was changed within either section.
